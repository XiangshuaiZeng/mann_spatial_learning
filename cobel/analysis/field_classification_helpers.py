...
def classify_place_field(activity_map):
    ...
    activity_map = np.where(activity_map < 0.50 * np.max(activity_map), 0,
                            activity_map)
    mask = activity_map > 0
...
def calculate_pdist(fields_sliced, place_indices):
    ...
    centers = np.zeros((place_indices.size, 6, 2))
...
def vector_cells(fields):
    ...
    ids = np.atleast_1d(ids.squeeze(axis=0))
