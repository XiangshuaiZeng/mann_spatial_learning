...
class FourConnectedGraphRotation(SpatialRepresentation):
    ...
    def __init__(self, modules: dict, graph_info: dict, step_size=1., act_type='egocentric'):
        '''
        ...
        # retrieve all the wall info from the environment, here self.wall_coordinates store the min and max XY of each wall,
        ...
        self.wall_coordinates, self.perimeter_nodes = world_module.get_wall_graph()
        ...
        self.top_info = {'nodes': nodes, 'edges': edges, 'walls': self.wall_coordinates}

        # indicate whether the action space is egocentric or allocentric
        self.act_type = act_type # egocentric | allocentric
        if act_type == 'egocentric':
            self.num_actions = 3
        elif act_type == 'allocentric':
            self.num_actions = 8
    ...
    def reset_start_nodes(self, start_nodes, ori=None):
        '''
    ...
    def generate_topology_from_worldInfo(self, step_size=1.0) -> (np.ndarray, np.ndarray):
        ...
        wall_edges = []
        for item in self.wall_coordinates:
            origin = np.array([item[0], item[1]])
            scale = np.array([item[2], item[3]])
            angle = -item[4] / 180 * np.pi
            # first compute the four points of the rectangle, assuming zero origin and rotation
            # the four points have to be arranged clock or counterclock wise
            four_points = [[scale[0] / 2, scale[1] / 2], [-scale[0] / 2, scale[1] / 2],
                           [-scale[0] / 2, -scale[1] / 2], [scale[0] / 2, -scale[1] / 2]]
            # apply rotation
            rot_matrix = np.array([[np.cos(angle), -np.sin(angle)], [np.sin(angle), np.cos(angle)]])
            four_points = four_points @ rot_matrix.T
            # move to the origin
            four_points += np.tile(origin, (4, 1))
            # store the four edges of the rectangle
            wall_edges.append([four_points[0], four_points[1]])
            wall_edges.append([four_points[1], four_points[2]])
            wall_edges.append([four_points[2], four_points[3]])
            wall_edges.append([four_points[3], four_points[0]])
        ...
        for xi in xv[0, :]:
            for yi in yv[:, 0]:
                ...
                distances = distances < (step_size / 3.5)
    ...
    def generate_behavior_from_action(self, action: int) -> dict:
        ...
        if action != 'reset':
            ...
            agent_angle = world_module.env_data['pose'][2]
            if self.act_type == 'egocentric':
                # lists for edges
                ...
                if action == 0:
                    ...
                    agent_angle += 90  # turn 90 degrees to the left
                    self.update_robot_pose(
                        [next_node_pos[0], next_node_pos[1], np.cos(agent_angle / 180 * np.pi), np.sin(agent_angle / 180 * np.pi)])
                elif action == 2:
            ...
            elif self.act_type == 'allocentric':
                # lists for edges
                north_edge, west_edge, south_edge, east_edge = [], [], [], []
                ...
                for n in neighbors:
                    if n.index != -1:
                        ...
                        vec_edge = neighbor_position - actual_node_position
                        vec_edge = vec_edge / np.linalg.norm(vec_edge)
                        angle = np.arctan2(vec_edge[1], vec_edge[0])
                        angle = angle / np.pi * 180.0
                        angle = self.normalize_angle(angle)
                        if angle == 0:
                            east_edge = [n.index, vec_edge, angle]
                        elif angle == 90:
                            north_edge = [n.index, vec_edge, angle]
                        elif angle == -90:
                            south_edge = [n.index, vec_edge, angle]
                        elif angle == 180 or angle == -180:
                            west_edge = [n.index, vec_edge, angle]
                # 8 possible movements, north, west, east, south, and rotate 90 to the 4 directions
                if action == 0:
                    if len(east_edge) != 0:
                        self.next_node = east_edge[0]
                    else:
                        self.next_node = self.modules[
                            'spatial_representation'].current_node
                        callback_value['hit_wall'] = True
                elif action == 1:
                    # this is a north movement
                    if len(north_edge) != 0:
                        self.next_node = north_edge[0]
                    else:
                        self.next_node = self.modules[
                            'spatial_representation'].current_node
                        callback_value['hit_wall'] = True
                    ...
                    if len(south_edge) != 0:
                        self.next_node = south_edge[0]
                    else:
                        self.next_node = self.modules[
                            'spatial_representation'].current_node
                        callback_value['hit_wall'] = True
                elif action == 4:
                    # this is a east turn movement
                    self.next_node = self.current_node
                    agent_angle = 0  # turn to 0 degrees due east
                elif action == 5:
                    # this is a east turn movement
                    self.next_node = self.current_node
                    agent_angle = 90  # turn to 90 degrees due north
                elif action == 6:
                    # this is a east turn movement
                    self.next_node = self.current_node
                    agent_angle = -180  # turn to -180 degrees due west
                elif action == 7:
                    # this is a east turn movement
                    self.next_node = self.current_node
                    agent_angle = -90  # turn to -90 degrees due south
            ...
            agent_angle = self.normalize_angle(agent_angle)
            ...
            if not self.offline:
                self.modules['world'].actuate_robot(np.array([next_node_pos[0], next_node_pos[1], agent_angle]))
            else:
                self.modules['world'].actuate_robot(np.array([self.next_node, agent_angle]))
            ...
            self.trajectories[-1].append([self.current_node, agent_angle])
