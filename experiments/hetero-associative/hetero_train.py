...
# plot function
def plot_predict_results(inputs1, inputs2, output1, targets2, outputs2):
    sequence_length = len(inputs1) - 1
...
def get_training_sequence(sequence_min_length, sequence_max_length, batch_size=1, test=False, valid_label=np.arange(0, 18)):
    # we only use the first 18 labels, which corresponds to the 10 digits and all uppercase letters
    ...
    # randomly select a few digit images
    encode_idx = []
    ...
    # input and output in the prediction stage; the last image will not be used as
    # input because it has no next-image.
    input2 = torch.zeros(sequence_length-1, batch_size, 1, img_dim[0], img_dim[1])
    output = torch.zeros(sequence_length-1, batch_size, 1, img_dim[0], img_dim[1])
    # randonly permute the previous image sequence
    per_idx = torch.randperm(sequence_length-1)
    input2 = torch.clone(encode_digits[per_idx])
    output = torch.clone(encode_digits[per_idx+1])
...
def train(model_path, epochs, param):
    ...
    optimizer = optim.RMSprop(model.parameters(), lr=1e-3, eps=1e-4, alpha=0.99)
...
def eval(model_path, test_epochs, param):
    ...
    for n in range(test_epochs):
        ...
        input1, input2, target2, per_idx, input_label = get_training_sequence(sequence_min_length, sequence_max_length, test=False)
        model.reset_hidden(param['keep_hidden'])
        model.clear_trace()
        ...
        for j in range(len(target2)):
            y_out2[j] = model(input2[j].unsqueeze(0))
        # plot_predict_results(torch.squeeze(input1), torch.squeeze(input2), torch.squeeze(y_out1),
        #                      torch.squeeze(target2),  torch.squeeze(y_out2))
        # plt.show()
        # log data
...
def eval_seq(model_path, test_epochs, param):
    param['batch_size'] = 1
    sequence_min_length = param['sequence_length_range'][0]
    sequence_max_length = param['sequence_length_range'][1]
    model = CnnFWM(param)
    ...
    model.eval()
    model.requires_grad_(False)
    test_log = {'memory_traces': [], 'input1': [], 'prediction_cue': [],  'prediction_self': [], 'target2': [],
                'per_idx':[], 'param': param, 'input_label': []}
    for n in range(test_epochs):
        if n % 10 == 0:
            print('Test episode: %s'%n)
        input1, input2, target2, per_idx, input_label = get_training_sequence(sequence_min_length, sequence_max_length, test=False)
        # first test the self-generated sequence
        model.reset_hidden(param['keep_hidden'])
        model.clear_trace()
        # stage 1: perception
        for j, vector in enumerate(input1[0:-1]):
            _ = model(vector.unsqueeze(0))
        # buffer point
        _ = model(input1[-1].unsqueeze(0))
        # stage 2: we offer the first input image and iterate to see if a sequence can be generated
        y_out_self = torch.zeros(target2.size())
        y_out_self[0] = model(input1[0].unsqueeze(0))
        for j in range(1, len(target2)):
            y_out_self[j] = model(y_out_self[j-1].unsqueeze(0))

        # then test the cue-generated sequence
        model.reset_hidden(param['keep_hidden'])
        model.clear_trace()
        # stage 1: perception
        for j, vector in enumerate(input1[0:-1]):
        ...
        _ = model(input1[-1].unsqueeze(0))
        # stage 2: we offer the first input image and iterate to see if a sequence can be generated
        y_out_cue = torch.zeros(target2.size())
        for j in range(len(target2)):
            y_out_cue[j] = model(input1[j].unsqueeze(0))
        ...
        test_log['memory_traces'].append(copy_trace)
        test_log['input1'].append(input1[0:-1].detach().cpu().numpy())
        test_log['prediction_cue'].append(y_out_cue.detach().cpu().numpy())
        test_log['prediction_self'].append(y_out_self.detach().cpu().numpy())
        test_log['target2'].append(target2.detach().cpu().numpy())
        test_log['per_idx'].append(per_idx.detach().cpu().numpy())
        test_log['input_label'].append(input_label)
    return test_log
...
def eval_general(model_path, test_epochs, param):
    ...
    for n in range(test_epochs):
        ...
        input1, input2, target2, per_idx, input_label = get_training_sequence(sequence_min_length, sequence_max_length,
                                                        test=False, valid_label=np.arange(0, 36))
        model.reset_hidden(param['keep_hidden'])
        model.clear_trace()
        target1 = torch.zeros(target2.size())
        target1 = input1[:-1]
...
if __name__ == "__main__":
    ...
    for simu in range(1, 6+1):
        seed = np.random.randint(0, 2 ** 32)
        set_seed(seed)
        model_path = "models/predict_cnn_fwm_%s_kv%s.pt" % (simu, activation)
        iftrain = True
        ifeval = True
        epochs = 50000
        test_epochs = 500
        ...
        if ifeval:
            ...
            #store testing data
            pickle.dump(test_log, open('data_test/predict_cnn_fwm_%s_kv%s.pkl' % (simu, activation),
                'wb'))

            # test_log = eval_seq(model_path, test_epochs, param)
            # pickle.dump(test_log, open('data_test/predict_cnn_fwm_%s_kv%s_seq.pkl' % (simu, activation),
            #       'wb'))

            # test_log = eval_general(model_path, test_epochs, param)
            # pickle.dump(test_log, open('data_test/predict_cnn_fwm_%s_kv%s_general.pkl' % (simu, activation),
            #       'wb'))
