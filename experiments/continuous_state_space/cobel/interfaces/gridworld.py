...
class InterfaceGridworld(AbstractInterface):
    ...
    def step(self, action: int) -> (int, float, bool, dict):
        ...
        Returns
        ----------
        current_state :                     The new current state.
    ...
    def reset(self) -> int:
        ...
        self.current_state = self.world['starting_states'][np.random.randint(self.world['starting_states'].shape[0])]
        # determine current coordinates
        self.current_coordinates = self.world['coordinates'][self.current_state]
        
        return self.current_state
    
    def update_transitions(self, invalid_transitions: list):
        '''
        This function updates the gridworld's transitions.
        
        Parameters
        ----------
        invalid_transitions :               The gridworld's invalid transitions as a list of 2-tuples.
        ...
        self.world['invalid_transitions'] = invalid_transitions
        ...
        for state in range(self.world['states']):
            for action in range(4):
                ...
                if next_state in self.world['invalid_states'] or (state, next_state) in self.world['invalid_transitions']:
                    next_state = state
    ...
    def init_visualization(self):
        ...
        if self.with_GUI:
            ...
            self.grid_plot = self.gui_parent.addPlot(title='Gridworld')
            ...
            for state in self.world['coordinates']:
                self.arrows.append(CogArrow(angle=0.0,headLen=20.0,tipAngle=25.0,tailLen=0.0,brush=(255,255,0)))
                self.arrows[-1].set_data(state[0] + 0.5, state[1] + 0.5, 0.)
    ...
    def update_visualization(self):
        ...
        if self.with_GUI:
            ...
            predictions = np.arange(self.world['states'])
            for p, prediction in enumerate(predictions):
                self.arrows[p].set_data(self.world['coordinates'][p][0] + 0.5, self.world['coordinates'][p][1] + 0.5, angle_table[np.argmax(prediction)])
            # update Qt visuals
            if hasattr(qt.QtGui, 'QApplication'):
                qt.QtGui.QApplication.instance().processEvents()
            else:
