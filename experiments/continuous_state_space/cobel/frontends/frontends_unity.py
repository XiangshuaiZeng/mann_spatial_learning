...
class FrontendUnityInterface():
    ...
    def __init__(self, scenario_name=None, side_channels=None, seed=42, timeout_wait=60, time_scale=3., worker_id=0):
        ...
        # command line args
        args = ["-batchmode"] ## Adding this it doesnt need to open a Unity GUI -JON
    ...
    def start_unity(self):
        self.worker_id = round(time.time()* 1000) % 1200
        args = ["-batchmode", "--mlagents-scene-name", self.scenario_name]
        self.env = UnityEnvironment(
            file_name=self.env_exec_path,
        ...
        ############################jon
    def change_scenario(self, scenario_name, env_exec_path):
        # Cierre del entorno actual si está abierto
        if self.env:
            self.stop_unity()

        # Actualización de la ruta del ejecutable y del nombre del escenario
        self.scenario_name = scenario_name
        self.env_exec_path = env_exec_path

        # Iniciar el nuevo entorno de Unity con la nueva configuración
        self.start_unity()
################################################################


    # ADDED TO MOVE THE REWARD OBJECT -JON (CHANGES IN UNITY)
    def step_simulation_with_object(self, x_agent: float, z_agent: float, yaw_agent: float, x_obj: float, z_obj: float,
                                    yaw_obj: float) -> (np.ndarray, np.ndarray):
        '''
        This function propels the simulation. It uses teleportation to guide the agent/robot and an object directly by means of global x, z, yaw values.

        Parameters
        ----------
        x_agent, z_agent, yaw_agent :       The global x, z, yaw positions to teleport the agent to.
        x_obj, z_obj, yaw_obj :             The global x, z, yaw positions to teleport the object to.
        ...
        '''
        # Combine agent and object actions into one array
        action = ActionTuple(np.asarray([[x_agent, z_agent, 90 - yaw_agent, x_obj, z_obj, 90 - yaw_obj]]))
    ...
    # MOVE THE REWARD TO XZ
    def move_object_only(self, x_obj: float, z_obj: float):
        '''
        This dunction move the object in this case the Reward; it is used a special value to identify that it is not the agent.
        '''
        specialValue = 999.0  # Valor especial para indicar movimiento del objeto
        action = ActionTuple(np.asarray([[x_obj, z_obj, specialValue]]))
        self.env.set_actions(self.behavior_name, action)
        # forward the simulation by a tick (and execute action)
        self.env.step()
...
class FrontendUnityOfflineInterface(FrontendUnityInterface):
    ...
    def __init__(self, scenario_path=None):
        ...
        #print("ENV",data)# Here env is a dict containing the topology idx as keys and corresponding images as values
        self.env = data
