...
class AgentFWM(nn.Module):
    ...
    def __call__(self, x):
        ...
        out = out.reshape(-1, out.size(-1)) #no entiendo porque
        act = self.h_c(out)
        val = self.h_v(out)
        # print('value:', val)
        # print('!!!!!!!!!!!!!!!!!!!!')
...
class AgentFWM_CNN(nn.Module):
    def __init__(self, p):
        ...
        self.merge = nn.Linear(756, p['in_size']).to(device) #656 for 3 action space , 756 for 4 action space
        self.relu = nn.ReLU().to(device)
        self.l1 = FWMRNN(p['in_size'], p['hidden'], p['k_size'], p['v_size'], p['num_reads']).to(device)
...
class PPO:
    ...
    def train(self, total_episodes=1000, episode_time_steps=250, update_length=200, checkpoint_path=''):
        ...
        while episode < total_episodes:
            ...
            self.time_step = 0
            # set up the goal visible at step 0 in unity env
            if self.time_step == 0:
                self.interface_OAI.modules_list[0]['spatial_representation'].reset_goal_nodes_Unity_ONLINE(
                        self.interface_OAI.modules_list[0]['spatial_representation'].goal_node,
                        0)
                #CHANGE
    ...
    def test(self, total_episodes=1000, episode_time_steps=250):
        ...
        while episode < total_episodes:
            ...
            while self.time_step < episode_time_steps:
                ...
                test_log['reached'][-1].append(self.interface_OAI.modules_list[0]['spatial_representation'].goal_reached) #goal_reached for only one reward #reached for multiple
                test_log['retrieving'][-1].append(self.interface_OAI.modules_list[0]['spatial_representation'].retrieving)
