...
class InterfaceMultiple():
    ...
    def step(self, action_list: list):
        ...
        for i in range(self.num_envs):
            ...
            callback_value['num_envs_id'] = i
            callback_value['actions'] = action_list[i]
    ...
    def reset_where(self, node) -> np.ndarray:
        '''
        AI Gym's reset function.
        Resets the environment and the agent's state to an specific node
        ...
        obs_idx = []
        for i in range(self.num_envs):
            self.modules_list[i]['spatial_representation'].generate_behavior_from_action('reset',node=1)
            self.observations[i] = np.copy(self.modules_list[i]['observation'].observation)
            pose = self.modules_list[i]['world'].env_data['pose']
            node, ori = pose[1], pose[2]
            idx = int(node * 4 + int(pose[2] / 90) % 4)
