...
class InterfaceBaseline(AbstractInterface):
    ...
    def step(self, action: int) -> (np.ndarray, float, bool, dict):
        ...
        '''
        callback_value = self.generate_behavior_from_action(action)
        ...
        return self.modules['observation'].observation, reward, end_trial, node_idx
    ...
    def reset(self) -> np.ndarray:
        ...
        observation :                       The observation of the new current state.
        '''
        self.generate_behavior_from_action('reset')
        self.observation = np.copy(self.modules['observation'].observation)
        node_idx = self.current_node
        
        return self.modules['observation'].observation, node_idx
    ...
    def get_position(self) -> np.ndarray:
        ...
        node_idx = self.current_node
        return np.array([current_node.x, current_node.y]), node_idx
