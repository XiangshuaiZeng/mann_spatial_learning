...
class ImageObservationBaseline():
    ...
    def update_world_module(self, new_world_module):
        """
        Update the reference to the world module and reset any necessary state
        or visualizations to match the new world environment.

        Parameters
        ----------
        new_world_module : The new world module after environment change.

        Returns
        ----------
        None
        """
        self.world_module = new_world_module
        ...
        if self.with_GUI:
            ...
            self.camera_image.setRect(
                QRectF(0.0, 0.0, image_scale, image_data.shape[0] / image_data.shape[1] * image_scale))
        # scale the one-line image to further reduce computational demand
    ...
    def update(self):
        ...
        # scale the images to lie in [-1, 1]
        observation = (observation-0.5)*2
    ...
    def get_observation_space(self):
        ...
        return gym.spaces.Box(low=0.0, high=1.0, shape=(self.image_dims[0], self.image_dims[1], 3))
