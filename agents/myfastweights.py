...
# This is the class to build the LSTM that feeds the FWM (this should be my actor-critic network)
class FWMRNN(nn.Module):
    def __init__(self, isize, hsize, withFWM=True, s_size=32):
        ...
        if withFWM:
            ...
            self.linear = nn.Linear(t_size, hsize).to(device)
    ...
    def forward(self, inputs, hidden):
        ...
        if self.hasFWM:
            for t, x_t in enumerate(x):
                F = self.fwm.write(x_t, F) # this is the update function of the FWM
                o_t = self.fwm(x_t, F)
                outputs.append(o_t)
...
# This is the class to build the actual memory
class FWM(nn.Module):
    def __init__(self, hidden_size, s_size, r_size, t_size):
        ...
        self.W_write = nn.Linear(hidden_size, 1 * write_size).to(device)

        # read
        self.W_read = nn.Linear(hidden_size, s_size + r_size * 3).to(device)
        ...
        self.m_trace = {'w_key': [], 'w_value': [], 'w_beta': [], 'r_key': [], 'r_value': []}
    ...
    def write(self, z, F):
        ...
        write_vars = self.W_write(z) # get the weight matrix of the FWM
        # write_vars: [batch_size, n_writes * (s_size + r_size + t_size + b_size)]

        write_sizes = [self.s_size, self.r_size, self.t_size, self.b_size]
        write_list = torch.split(write_vars, sum(write_sizes), dim=1) # split the weight matrix into single weight matrices for the key, value, query and beta?
        # write_list: list of [batch_size, s_size + r_size + t_size + b_size]

        # multiple writes at once (multi-head attention?)
        scale = 1.#/self.t_size
